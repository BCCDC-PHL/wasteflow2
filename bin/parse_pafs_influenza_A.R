...
if (length(args)!=5) {
  stop("5 arguments must be supplied: flu info db, paf file, sample name, out directory, score threshold.", call.=FALSE)
...
assigment_dt <- merge_dt %>%
  ...
  summarize(tot_read_length = sum(qlength),
            ...
            align_score = ANI * AF, .groups = 'drop') %>%
...
sum_dt <- assigment_dt %>%
  ...
  summarize(n = n(),
            ...
            avg_score = mean(align_score), .groups = 'drop') %>%
