...
if (!all(file.exists(INPUT_FILES))) {
    stop(
        paste(
            "The following input files don't exist:",
            paste(
...
if (ncol(dat) == 6) {
    ...
    dat$region <- factor(
        dat$region,
        levels = unique(dat$region[order(dat$start)])
    )
} else {
...
for (sample in unique(dat$sample)) {
    ...
    if (ncol(sample_dat) == 6) {
        ...
        ggsave(
            file = outfile,
            plot,
            height = 3 + (0.2 * length(unique(sample_dat$region))),
            width = 16,
            units = "cm",
            limitsize = FALSE
        )
