...
class Metadata:
    def __init__(self, metadata_file):
        ...
        if "sample_id" in self.df.columns:
            self.df["sample_id"] = self.df["sample_id"].str.replace(" ", "-", regex=False)
        else:
            print("ERROR: 'sample_id' column not found after cleaning.", file=sys.stderr)
            sys.exit(1)

        # 2. season should be YYYY or YYYY_YYYY
        self.df["season"] = self.df["season"].apply(
            lambda x: x if pd.isna(x)
            else (x if re.match(r"^\d{4}(_\d{4})?$", x) else pd.NA)
        )
        ...
        self.df["period"] = self.df["period"].where(self.df["period"].isin(["off_season", "in_season"]))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Metadata parser and cleaner for SAFEGUARD workflow.")
    parser.add_argument("--metadata_file", required=True, help="Path to metadata TSV file")
    parser.add_argument("--filter", nargs="*", help="Optional filters, e.g. season=2025_2026 period=in_season extraction_source=research")
