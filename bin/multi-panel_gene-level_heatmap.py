...
def main():
    ...
    plot_heatmap_with_breadth(df_h1n1_cov, df_h1n1_breadth, "Influenza A (H1N1)", ax=axes[0], bold_threshold=0.5, x_labels=x_labels_h1n1)
    plot_heatmap_with_breadth(df_h3n2_cov, df_h3n2_breadth, "Influenza A (H3N2)", ax=axes[1], bold_threshold=0.5, x_labels=x_labels_h3n2)
    plot_heatmap_with_breadth(df_h5n1_cov, df_h5n1_breadth, "Influenza A (H5N1)", ax=axes[2], bold_threshold=0.5, x_labels=x_labels_h5n1)
    fig.text(0.5,0.02,"Cells with black borders indicate segments where ≥50% of the segment has ≥5× coverage", ha="center", fontsize=10)
    plt.tight_layout(rect=[0,0.05,1,1])
