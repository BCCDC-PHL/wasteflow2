...
def main():
    ...
    if args.metadata and args.batch:
        ...
        # Remove samples marked as Ordered in Error (case-insensitive)
        meta = meta[~meta['Results'].str.lower().eq("ordered in error")].copy()

        # Clean up IDs
        meta['Sample_ID_clean'] = meta['Sample_ID'].astype(str).str.replace(' ', '-')
        meta['Collection Date'] = pd.to_datetime(meta['Collection Date'])

        # Sort by patient then collection date
        ...
        x_labels = meta_sorted.apply(
            lambda r: f"{r['Patient First Name']}\n{r['Collection Date'].strftime('%m/%d')}", 
            axis=1
        ).tolist()
    else:
