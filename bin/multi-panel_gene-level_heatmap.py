...
def load_bed(file_path):
    bed = pd.read_csv(
        file_path, sep="\t", header=None, names=["chrom", "start", "end", "gene"]
    )
    return bed
...
def compute_gene_coverage(mosdepth_df, bed_df, sample_order):
    ...
    for _, row in bed_df.iterrows():
        cov_rows = mosdepth_df[
            (mosdepth_df["start"] < row["end"]) & (mosdepth_df["end"] > row["start"])
        ]
        if not cov_rows.empty:
            ...
            gene_cov[row["gene"]] = pd.Series(
                [0] * len(sample_order), index=sample_order
            )
            gene_breadth[row["gene"]] = pd.Series(
                [0] * len(sample_order), index=sample_order
            )
...
def plot_heatmap_with_breadth(
    ...
    cmap = LinearSegmentedColormap.from_list(
        "white_seagreen", ["white", "lightgreen", "seagreen"]
    )
    ...
    sns.heatmap(
        ...
        vmax=vmax,
        annot=False,
        cbar_kws={"label": "Mean coverage (X)"},
    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Multi-panel gene-level coverage heatmap with metadata filtering"
    )
    parser.add_argument(
        "--sars", required=False, help="SARS-CoV-2 coverage file (mosdepth)"
    )
    ...
    parser.add_argument(
        "--metadata", required=False, help="Optional metadata file (tsv)"
    )
    parser.add_argument(
        "--filter",
        required=False,
    ...
    if args.metadata:
        ...
        meta_filtered["collection_date"] = pd.to_datetime(
            meta_filtered["collection_date"]
        )

        # Define custom site order for sorting
        site_order = ["NHA", "IHA", "MV", "VIHA"]
        meta_filtered["site_prefix"] = (
            meta_filtered["collection_site"].str.split("-").str[0]
        )
        meta_filtered["site_prefix"] = pd.Categorical(
            meta_filtered["site_prefix"], categories=site_order, ordered=True
        )

        # Sort first by date, then by site order
        meta_sorted = meta_filtered.sort_values(
            ["collection_date", "site_prefix", "collection_site"]
        )

        sample_list = meta_sorted["sample_id"].tolist()
        x_labels = meta_sorted.apply(
            lambda r: f"{r['collection_site']}\n{r['collection_date'].strftime('%m/%d')}",
            axis=1,
        ).tolist()
    else:
        ...
        if args.sars:
            ...
            raise ValueError(
                "No metadata or coverage files provided → cannot infer sample list"
            )
    ...
    # ----------------------
    if args.sars:
        df_sars_cov, df_sars_breadth = compute_gene_coverage(
            load_mosdepth(args.sars), load_bed(args.sars_bed), sample_list
        )
    else:
    ...
    # RSV-A
    if args.rsvA:
        df_rsvA_cov, df_rsvA_breadth = compute_gene_coverage(
            load_mosdepth(args.rsvA), load_bed(args.rsvA_bed), sample_list
        )
    else:
    ...
    # RSV-B
    if args.rsvB:
        df_rsvB_cov, df_rsvB_breadth = compute_gene_coverage(
            load_mosdepth(args.rsvB), load_bed(args.rsvB_bed), sample_list
        )
    else:
    ...
    plot_heatmap_with_breadth(
        df_sars_cov, df_sars_breadth, "SARS-CoV-2", ax=axes[0], x_labels=x_labels
    )
    plot_heatmap_with_breadth(
        df_rsvA_cov, df_rsvA_breadth, "RSV-A", ax=axes[1], x_labels=x_labels
    )
    plot_heatmap_with_breadth(
        df_rsvB_cov, df_rsvB_breadth, "RSV-B", ax=axes[2], x_labels=x_labels
    )

    fig.text(
        0.5,
        0.02,
        "Cells with black borders indicate genes where ≥75% of the gene has ≥5× coverage",
        ha="center",
        fontsize=10,
    )
