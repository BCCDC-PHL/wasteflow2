...
def parse_args():
    parser = argparse.ArgumentParser(description='Plot QC results for viral and control oligos')
    parser.add_argument('--input_file', required=True, help='Input TSV file with concatenated idxstats')
    parser.add_argument('--control_ref', default='CTRL_OLIGO', help='Control oligo reference ID')
    parser.add_argument('--out_prefix', default='qc_results', help='Prefix for output files')
...
def main():
    ...
    plot_df = reads[[f'{ref}_prop' for ref in viral_refs] + ['Control_prop','Unmapped_prop']]
    ...
    plt.title("Proportion of Viral, Control, and Unmapped Reads per Sample\n(Total reads shown above each bar)")
    plt.xticks(rotation=90, ha='right')
    #plt.legend([*viral_refs, "Control", "Unmapped"])
    plt.legend([*viral_refs, "Control", "Unmapped"], loc='center left', bbox_to_anchor=(1, 0.5))
    plt.tight_layout()
